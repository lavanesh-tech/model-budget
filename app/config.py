...
class Settings(BaseSettings):
    ...
    otel_service_name: str = "model-budget"
    ...
    @field_validator("otel_service_name", "otel_exporter_otlp_endpoint")
    @classmethod
    def _otel_values_not_blank(cls, value: str | None) -> str | None:
        if value is not None and not value.strip():
            raise ValueError("OpenTelemetry string settings must not be blank when provided")
        return value
